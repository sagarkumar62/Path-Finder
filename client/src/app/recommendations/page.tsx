        ...
        {recommendations.length === 0 ? (
          <Card className="p-8 text-center space-y-4 bg-white border-slate-200 shadow-soft rounded-2xl">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-500 max-w-md mx-auto">
                Complete your learner onboarding to let our AI engine analyze your skill set and generate personalized career recommendations.
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {recommendations.map((car, idx) => {
              const carId = car?.id || `car_${idx}`;
              const carTitle = car?.title || (car as any)?.career || 'Career Goal';
              const isCompared = compareIds.includes(carId);
              const isActiveGoal = Boolean(
                currentActiveGoalTitle && carTitle.toLowerCase() === currentActiveGoalTitle.toLowerCase()
              );
              const whyMatches = Array.isArray(car?.whyMatches) ? car.whyMatches : Array.isArray((car as any)?.reasons) ? (car as any).reasons : [];
              const skillGaps = Array.isArray(car?.skillGaps) ? car.skillGaps : [];
              ...
              return (
                ...
                >
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        <div className="flex items-center gap-2">
                          <h2 className="font-extrabold text-xl text-slate-900">{carTitle}</h2>
                          {isActiveGoal && <Badge variant="ai">Active Goal</Badge>}
                        </div>
                        <div className="flex items-center gap-2 mt-1">
                          <Badge variant="primary">{car?.difficulty || 'Intermediate'} Difficulty</Badge>
                          <span className="text-xs font-medium text-slate-500">{car?.estimatedTransition || '3-6 Months'}</span>
                        </div>
                      </div>
                      <MatchScore score={car?.matchScore || 85} size="sm" showLabel={false} />
                    </div>

                    <p className="text-xs text-slate-600 leading-relaxed">{car?.description || 'Tailored career recommendation matching your current skill matrix.'}</p>
                    ...
                    {whyMatches.length > 0 && (
                      <div className="space-y-2 p-3 rounded-xl bg-slate-50 border border-slate-100">
                        ...
                        <ul className="space-y-1 text-xs text-slate-700 font-medium">
                          {whyMatches.map((reason: string, i: number) => (
                    ...
                    {skillGaps.length > 0 && (
                      <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {skillGaps.map((sg: string, i: number) => (
                  ...
                  <div className="pt-4 border-t border-slate-100 flex flex-col gap-2">
                    <div className="flex items-center gap-2">
                      <Link href={`/careers/${carId}`} className="flex-1">
                    ...
                    <Button
                      ...
                      size="sm"
                      disabled={settingGoalId === carId}
                      onClick={() => handleSetActiveGoal(carId, carTitle)}
